...
class Slider{
	...
	initUI(){
		...
		//2)、创建豆豆：
		let ulDom = $create("ul");
		ulDom.style.cssText = "position:absolute;left:450px;bottom:0;height:40px;list-style:none;z-index:999;";
		this.boxDom.appendChild(ulDom);
		for(let i=0;i<this.imgs.length;i++){
			let liDom = $create("li");
			liDom.style.cssText = "float:left;margin-left:10px;width:10px;height:10px;border-radius:50%;background-color:#b6b6b6;";
